...
#define FLASHCACHE_SW_VERSION "flashcache-3.1.1-wjl-test-0.0.3"
...
flashcache_do_io(struct kcached_job *job)
{
	...
	job->dmc->flashcache_stats.ssd_replaces++;
...
//==============================
static void quicksort(sector_t* nums, int left, int right) {
    sector_t flag = nums[left];
    int i = left, j = right;
    while (i < j) {     
...
static void insert_lwc_whitelist(struct cache_c *dmc, struct cache_set *set, sector_t dbn) {
	set->lwc_whitelist[set->lwc_whitelist_pos] = dbn;
	set->lwc_whitelist_pos++;
	set->lwc_whitelist_pos %= dmc->sysctl_lwc_whitelist_size;
	if (set->lwc_whitelist_len < dmc->sysctl_lwc_whitelist_size) {
		set->lwc_whitelist_len++;
	}
}

static void dedup(struct cache_c *dmc, struct cache_set *set, sector_t* nums, int left, int right) {
	int i, count = 1;
	sector_t last = nums[left];
	for (i = left + 1; i <= right; i++) {
		if (nums[i] == last) {
			nums[i] = -1;
			count++;
			if (count == dmc->sysctl_lwc_window_threshold) {
				insert_lwc_whitelist(dmc, set, last);
			}
		}
		else {
			last = nums[i];
...
/* ADDED by shao for LWC */

/* insert/update the whitesheet list */
static void update_lwc_whitelist(struct cache_c *dmc, unsigned long set_number)
{
	struct cache_set *set;
	int i, j, start, end, start2, end2, p1, p2;
	set = &dmc->cache_sets[set_number];

	//====================================
	dmc->flashcache_stats.lwc_whitesheet_writes++;
	if (dmc->sysctl_lwc_policy == 0) {
		// policy by Shao
		for (i = 0; i < dmc->sysctl_lwc_window_size * (dmc->sysctl_lwc_window_num - 1) - 1; i++)
		{
			for (j = i + dmc->sysctl_lwc_window_size + 1; j < dmc->sysctl_lwc_window_size * dmc->sysctl_lwc_window_num; j++)
			{
				if(set->array_temp[i] == set->array_temp[j])
				{	
				/*	
					dmc->mywhitesheet[dmc->mywhitesheet_pos] = set->array_temp[j];			
					dmc->mywhitesheet_pos++;
					dmc->mywhitesheet_pos = dmc->mywhitesheet_pos % WHITE_SHEET_LENGTH;
					if(dmc->mywhitesheet_len < WHITE_SHEET_LENGTH)
					{
						dmc->mywhitesheet_len++;	
					}
				*/
					insert_lwc_whitelist(dmc, set, set->array_temp[i]);
				}
			}
		}
	}
	else if (dmc->sysctl_lwc_policy == 1) {
		/**
		* in this policy, if dbn appear in more than 2 window, add into white list
		*/
		for (i = 0; i < dmc->sysctl_lwc_window_num; i++) {
			start = i * dmc->sysctl_lwc_window_size;
			end = start + dmc->sysctl_lwc_window_size - 1;
			quicksort(set->array_temp, start, end);
			dedup(dmc, set, set->array_temp, start, end);
		}
		for (i = 0; i < dmc->sysctl_lwc_window_num; i++) {
			start = i * dmc->sysctl_lwc_window_size;
			end = start + dmc->sysctl_lwc_window_size;
			for (j = i + 1; j < dmc->sysctl_lwc_window_num; j++) {
				start2 = i * dmc->sysctl_lwc_window_size;
				end2 = start + dmc->sysctl_lwc_window_size;
				p1 = start;
				p2 = start2;
				while (p1 < end && p2 < end2) {
					while (p1 < end && set->array_temp[p1] == -1) {
						p1++;
					}
					if (p1 >= end) {
						break;
					}
					while (p2 < end2 && set->array_temp[p2] == -1) {
						p2++;
					}
					if (p2 >= end2) {
						break;
					}
					if (set->array_temp[p1] == set->array_temp[p2]) {
						insert_lwc_whitelist(dmc, set, set->array_temp[p1]);
						set->array_temp[p1] = -1;
						set->array_temp[p2] = -1;
						p1++;
						p2++;
					}
					else if (set->array_temp[p1] > set->array_temp[p2]) {
						p2++;
					}
					else {
						p1++;
...
/* re-initial array list */
static void array_init(struct cache_c *dmc, struct cache_set *set)
{
	memset(set->array_temp, 0, dmc->sysctl_lwc_window_size * dmc->sysctl_lwc_window_num * sizeof(sector_t));
	set->arrayIndex = 0;	
}
/* */
...
//==============================
/* find in whitesheet list */
static int whitesheet_find(struct cache_c *dmc, sector_t dbn, unsigned long set_number)
{
	int i;
	struct cache_set *set;
	set = &dmc->cache_sets[set_number];
	for (i = 0; i < set->lwc_whitelist_len; i++) {
		if (dbn == set->lwc_whitelist[i]) {
			dmc->flashcache_stats.lwc_hits++;
			// delete hit block
			/*
			if (set->lwc_whitelist_pos > 0) {
				set->lwc_whitelist[i] = set->lwc_whitelist[set->lwc_whitelist_pos - 1];
				set->lwc_whitelist_pos--;
				if (set->lwc_whitelist_len != dmc->sysctl_lwc_whitelist_size) {
					set->lwc_whitelist_len--;
				}
	...
	/*
	for(i = 0; i < dmc->mywhitesheet_len; i++)
	{
		if (dbn == dmc->mywhitesheet[i])
		{
			dmc->flashcache_stats.lwc_hits++;
			return VALID;
		}
...
/* add a dbn to temp array list */
static void array_add(struct cache_c *dmc, sector_t dbn, unsigned long set_number)
{	
	struct cache_set *set;
	set = &dmc->cache_sets[set_number];
	set->array_temp[set->arrayIndex] = dbn;
	set->arrayIndex++;	
	
	if(set->arrayIndex >= dmc->sysctl_lwc_window_size * dmc->sysctl_lwc_window_num )
	{
		update_lwc_whitelist(dmc, set_number);	
		array_init(dmc, set);
	}
}
/* */
...
flashcache_lookup(struct cache_c *dmc, struct bio *bio, int *index)
{
	...
	if (invalid == -1) {
		// ========================
		// check if it is in the lwc_whitelist, only block in whitelist will be replaced into SSD. 
		if(dmc->sysctl_lwc_enable) {		
			array_add(dmc, dbn, set_number);
			ret = whitesheet_find(dmc, dbn, set_number);
			if(ret == INVALID) {
				dmc->flashcache_stats.lwc_invalid_reject++;
				return -1;
			}
		}
		// ========================
...
#endif
{
	...
	//===============
	dmc->flashcache_stats.bio_arrival++;

	if (bio_data_dir(bio) == READ) 
	{
		dmc->flashcache_stats.reads++;
		dmc->flashcache_stats.reads_shadow++;
	}
	else
	{
		dmc->flashcache_stats.writes++;
		dmc->flashcache_stats.writes_shadow++;
	}
	//===============
