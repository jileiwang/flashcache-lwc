...
init:	
	...
	/* ADDED by shao */		
	dmc->mywhitesheet_len = 0;
	dmc->mywhitesheet_pos = 0;
	dmc->mywhitesheet = (sector_t *)vmalloc(WHITE_SHEET_LENGTH * sizeof(sector_t));
	memset(dmc->mywhitesheet, 0, WHITE_SHEET_LENGTH * sizeof(sector_t));
	/* */

	//===========================
	dmc->sysctl_lwc_whitelist_size = 50;
	dmc->sysctl_lwc_window_size = 52;
	dmc->sysctl_lwc_window_num = 3;
	dmc->sysctl_lwc_window_threshold = 3;
	dmc->sysctl_lwc_policy = 0;
	//===========================
	...
	for (i = 0 ; i < dmc->num_sets ; i++) {
		...
		//======================
		dmc->cache_sets[i].arrayIndex = 0;
		dmc->cache_sets[i].array_temp = (sector_t *)vmalloc(dmc->sysctl_lwc_window_size * dmc->sysctl_lwc_window_size * sizeof(sector_t));
		memset(dmc->cache_sets[i].array_temp, 0, dmc->sysctl_lwc_window_size * dmc->sysctl_lwc_window_size * sizeof(sector_t));
				
		dmc->cache_sets[i].lwc_whitelist = (sector_t *)vmalloc(sizeof(sector_t) * dmc->sysctl_lwc_whitelist_size);
		dmc->cache_sets[i].lwc_whitelist_pos = 0;
		dmc->cache_sets[i].lwc_whitelist_len = 0;
		//======================
		/* */
		       ...
		       char *result, unsigned int maxlen)
{
	...
	//===================
	DMEMIT("\tbio_arrival(%lu), reads_shadow(%lu), writes_shadow(%lu)\n", 
	       stats->bio_arrival, stats->reads_shadow, stats->writes_shadow);
	//===================
	...
	DMEMIT("\tdisk reads(%lu), disk writes(%lu) ssd reads(%lu) ssd writes(%lu)\n" \
		...
		"\tlwc hits(%lu), lwc whitesheet writes(%lu), lwc invalid reject(%lu), ssd replaces(%lu)\n" \
		"\tdisk read errors(%d), disk write errors(%d) ssd read errors(%d) ssd write errors(%d)\n" \
	       "\tuncached sequential reads(%lu), uncached sequential writes(%lu)\n" \
	       "\tpid_adds(%lu), pid_dels(%lu), pid_drops(%lu) pid_expiry(%lu)\n" \
	       ...
	       stats->disk_reads, stats->disk_writes, stats->ssd_reads, stats->ssd_writes,
	       stats->uncached_reads, stats->uncached_writes, stats->uncached_io_requeue,
               stats->lwc_hits, stats->lwc_whitesheet_writes, stats->lwc_invalid_reject, stats->ssd_replaces,
		 dmc->flashcache_errors.disk_read_errors, dmc->flashcache_errors.disk_write_errors, dmc->flashcache_errors.ssd_read_errors, dmc->flashcache_errors.ssd_write_errors,
	       stats->uncached_sequential_reads, stats->uncached_sequential_writes,
	       stats->pid_adds, stats->pid_dels, stats->pid_drops, stats->expiry,
